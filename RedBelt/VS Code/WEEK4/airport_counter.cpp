...
public:
  ...
  {
    if (airports[static_cast<uint32_t>(airport)] > 0)
      --airports[static_cast<uint32_t>(airport)];
  }
  ...
  {
    Items items;
    uint32_t last = static_cast<uint32_t>(TAirport::Last_);
    uint32_t i = 0;
    do
    {
...
private:
  array<uint32_t, static_cast<uint32_t>(TAirport::Last_)> airports;
};
